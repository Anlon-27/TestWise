...
def get_extract_path():
    """
    获取当前进程使用的 extract 变量文件路径。
    pytest-xdist 并行时每个 worker 通过 EXTRACT_FILE 环境变量使用独立文件，避免互相覆盖。
    """
    return os.environ.get('EXTRACT_FILE') or FILE_PATH['EXTRACT']
...
class ReadYamlData:
    ...
    def clear_yaml_data(self):
        ...
        :return:
        """
        with open(get_extract_path(), 'w') as f:
            f.truncate()
    ...
    def get_extract_yaml(self, node_name, second_node_name=None):
        ...
        if os.path.exists(extract_path):
            ...
            file = open(extract_path, 'w')
            file.close()
            logs.info('extract.yaml创建成功！')
        try:
            with open(extract_path, 'r', encoding='utf-8') as rf:
