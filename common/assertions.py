...
class Assertions:
    ...
    def contains_assert(self, value, response, status_code):
        ...
        for assert_key, assert_value in value.items():
            if assert_key == "status_code":
                ...
                if not resp_list:
                    flag += 1
                    allure.attach(f"接口返回中未找到字段：{assert_key}", '响应文本断言结果：失败',
                                  attachment_type=allure.attachment_type.TEXT)
                    logs.error("contains断言失败：接口返回中未找到字段【%s】", assert_key)
                    continue
    ...
    def assert_schema(self, expected, response):
        """
        响应结构契约断言
        :param expected: 期望结构，如 {'required': {'error_code': str}, 'optional': {'token': str}}
        :param response: 接口实际响应
        :return: flag，0 表示通过，非 0 表示失败
        """
        from common.schema_check import validate_schema

        ok, errors = validate_schema(response, expected)
        if ok:
            logs.info("schema断言成功")
            allure.attach("", "schema断言结果：成功", allure.attachment_type.TEXT)
            return 0
        msg = "; ".join(errors)
        logs.error("schema断言失败：%s", msg)
        allure.attach(msg, "schema断言结果：失败", allure.attachment_type.TEXT)
        return 1
            ...
            for yq in expected:
                for key, value in yq.items():
                    if key == "contains":
                        ...
                        flag = self.assert_schema(value, response)
                        all_flag = all_flag + flag
                    else:
