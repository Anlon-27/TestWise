...
class ConnectClickHouse:
    ...
    def __init__(self):
        ...
        if clickhouse_sqlalchemy is None or make_session is None:
            logs.error("未安装 clickhouse-sqlalchemy，无法使用 ClickHouse 连接。可执行: uv sync 或 pip install clickhouse-sqlalchemy")
            return
