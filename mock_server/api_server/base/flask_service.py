...
def read_orders():
    """读取订单数据，兼容旧的单对象格式，返回订单列表。"""
    try:
        data = json.loads(read_data(DIR_BASE + '/data/mockdata/orderNumber.json'))
    except Exception:
        return []
...
def append_order(order_num, user_id):
    """追加一笔订单，保证并行下单互不覆盖。"""
    orders = read_orders()
    orders.append({'order_num': order_num, 'user_id': user_id})
    write_data(DIR_BASE + '/data/mockdata/orderNumber.json', json.dumps(orders, ensure_ascii=False))
...
def add_user():
    ...
    if all([username, password, role_id, dates, phone]) and token in issued_tokens:
...
def place_an_order():
    ...
    if all([goods_id, number, propertyChildIds, inviter_id, price, freight_insurance, discount_code]):
        if goods_id in good_id_list:
            ...
            append_order(order_num, user_id)
            response = {
...
def order_pay():
    ...
    if all([order_num, user_id]):
        matched = any(
            str(o.get('order_num')) == str(order_num) and str(o.get('user_id')) == str(user_id)
            for o in orders
        )
        if matched:
            response = {
...
def check_order_status():
    ...
    if any(str(o.get('order_num')) == str(order_number) for o in orders):
        response = {
            'status': '0',
            'queryTime': now_date(),
            'error': '',
...
if __name__ == '__main__':
    ...
    # threaded=True，支持 pytest-xdist 并行测试时的并发请求
    api.run(host='127.0.0.1', port=8787, debug=True, threaded=True)
